...
var debugx = Devebot.require('debug')('appTokenify:handler');
...
var Service = function (params) {
  ...
  self.verifyHttpAuth = function (req, res, next) {
    if (httpauthCfg.enabled == false && process.env.NODE_ENV != 'production') {
      ...
      logger.debug('The HttpAuth verification is bypassed in NODE_ENV[%s] - Request[%s]', process.env.NODE_ENV, req.traceRequestId);
      req[pluginCfg.sessionObjectName] = { enabled: false };
      return next();
    }
    verifyHttpAuth(req).then(function () {
      logger.debug('HttpAuth verification passed - Request[%s]', req.traceRequestId);
      next();
    }).catch(function (error) {
      debugx.enabled && debugx(' - HttpAuth verification failed, return an error code 401');
      logger.debug('HttpAuth verification failed, return 401 - Request[%s]', req.traceRequestId);
  ...
  var verifyHttpAuth = verifier['httpauth'] = function (req) {
    ...
    return params.tokenifyStorage.authenticate(credential, {
      ...
      if (result.status == 0) {
        ...
        logger.debug('Created sessionObject:%s - Request[%s]', JSON.stringify(sessionObject), req.traceRequestId);
  ...
  self.verifyToken = function (req, res, next) {
    if (tokenCfg.enabled == false && process.env.NODE_ENV != 'production') {
      ...
      logger.debug('The Token verification is bypassed in NODE_ENV[%s] - Request[%s]', process.env.NODE_ENV, req.traceRequestId);
      req[pluginCfg.sessionObjectName] = { enabled: false };
      return next();
    }
    verifyToken(req).then(function () {
      logger.debug('Token verification passed - Request[%s]', req.traceRequestId);
      next();
    }).catch(function (error) {
      debugx.enabled && debugx(' - Token verification failed, return an error code 401');
      logger.debug('Token verification failed, return 401 - Request[%s]', req.traceRequestId);
      if (lodash.isObject(error)) {
        res.status(401).json(error);
      } else {
        res.status(401).send({ success: false, message: error });
  ...
  self.authenticate = function (req, res, next) {
    ...
    logger.debug('Client authenticate username:%s - Request[%s]', req.body.username, req.traceRequestId);
    ...
    params.tokenifyStorage.authenticate(credential, {
      ...
      if (result.status == 0) {
        ...
        logger.debug('Successful authentication. Created token:%s - Request[%s]', token, req.traceRequestId);
        res.json({
          success: true,
      ...
      logger.debug('Authentication failed. status:%s - Request[%s]', result.status, req.traceRequestId);
      res.json({
        success: false,
      ...
      logger.debug('Authentication failed. status:%s - Request[%s]', error.status, req.traceRequestId);
      error.success = false;
      res.status(400).json(error);
    }).finally(function () {
      logger.debug('Authentication finish - Request[%s]', req.traceRequestId);
    });
  };
  ...
  self.verifyJWT = function (req, res, next) {
    if (jwtCfg.enabled == false && process.env.NODE_ENV != 'production') {
      ...
      logger.debug('The JWT verification is bypassed in NODE_ENV[%s] - Request[%s]', process.env.NODE_ENV, req.traceRequestId);
      req[pluginCfg.sessionObjectName] = { enabled: false };
      return next();
    }
    verifyJWT(req).then(function () {
      logger.debug('JWT verification passed - Request[%s]', req.traceRequestId);
      next();
    }).catch(function (error) {
      debugx.enabled && debugx(' - JWT verification failed, return an error code 403');
      logger.debug('JWT verification failed, return 403 - Request[%s]', req.traceRequestId);
      if (lodash.isObject(error)) {
  ...
  var verifyJWT = verifier['jwt'] = function (req) {
    ...
    if (token) {
      logger.debug('JWT token found: [%s] - Request[%s]', token, req.traceRequestId);
      var tokenOpts = {
        ignoreExpiration: jwtCfg.ignoreExpiration || false
      };
      debugx.enabled && debugx(' - decode token, verifies secret and checks exp');
      logger.debug('Call jwt.verify() with options: %s - Request[%s]', JSON.stringify(tokenOpts), req.traceRequestId);
      return new Promise(function (resolve, reject) {
        jwt.verify(token, jwtCfg.secretkey || 't0ps3cr3t', tokenOpts, function (err, decoded) {
          if (err) {
            ...
            logger.debug('Verification failed, error: %s - Request[%s]', JSON.stringify(err), req.traceRequestId);
            return reject({
              success: false,
              type: 'JWT',
            ...
            logger.debug('Verification success, token: %s - Request[%s]', JSON.stringify(decoded), req.traceRequestId);
      ...
      logger.debug('JWT token not found - Request[%s]', req.traceRequestId);
      return Promise.reject({
        success: false,
        type: 'JWT',
  ...
  self.verifyKST = function (req, res, next) {
    if (kstCfg.enabled == false && process.env.NODE_ENV != 'production') {
      ...
      logger.debug('The KST verification is bypassed in NODE_ENV[%s] - Request[%s]', process.env.NODE_ENV, req.traceRequestId);
      req[pluginCfg.sessionObjectName] = { enabled: false };
      return next();
    }
    verifyKST(req).then(function () {
      logger.debug('KST verification passed - Request[%s]', req.traceRequestId);
      next();
    }).catch(function (error) {
      debugx.enabled && debugx(' - KST verification failed, return an error code 403');
      logger.debug('KST verification failed, return 403 - Request[%s]', req.traceRequestId);
      if (lodash.isObject(error)) {
        res.status(403).json(error);
      } else {
        res.status(403).send({ success: false, message: error });
  ...
  var verifyKST = verifier['kst'] = function (req) {
    ...
    if (authHeaders.key && authHeaders.nonce && authHeaders.timestamp && authHeaders.signature) {
      ...
      logger.debug('KST token is invalid - Request[%s]', req.traceRequestId);
      return Promise.reject({
        success: false,
        type: 'KST',
        message: 'Token not found'
  ...
  self.verifyMIX = function (authMethods) {
    ...
    return function (req, res, next) {
      logger.debug('MIX verification: %s - Request[%s]', JSON.stringify(mixtureAuthMethods), req.traceRequestId);
      ...
      Promise.any(lodash.map(mixtureAuthMethods, function (authMethod) {
        ...
        logger.debug('The MIX verification passed - Request[%s]', req.traceRequestId);
        next();
        ...
        logger.debug('The MIX verification failed, return 401 - Request[%s]', req.traceRequestId);
        res.set('WWW-Authenticate', 'Basic realm="tokenify"');
        res.set('Content-Type', 'application/json');
        res.status(401).send({
          success: false,
