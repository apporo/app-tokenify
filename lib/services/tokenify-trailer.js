      ...
      if (lodash.isArray(mixCfg)) {
        mixCfg.forEach(function(mixItem, index) {
          ...
          if (mixItem.enabled != false && lodash.isArray(protectedPaths) && !lodash.isEmpty(protectedPaths)) {
            webserverTrigger.inject(params.tokenifyService.verifyMIX(mixItem.authMethods),
                protectedPaths, position.POSITION_TOKENIFY, 'app-tokenify-mix#' + index);
          }
        });
      }
    }

    if (pluginCfg.authorization && pluginCfg.authorization.enabled != false) {
      webserverTrigger.inject(params.tokenifyHandler.buildPermissionChecker(express),
          null, position.POSITION_TOKENIFY, 'app-tokenify-intercept-url');
    }
  }
