...
var Service = function(params) {
  ...
  if (lodash.isArray(protectedPaths_jwt) && !lodash.isEmpty(protectedPaths_jwt)) {
    webserverTrigger.inject(params.tokenifyService.verifyJWT,
        protectedPaths_jwt, position.POSITION_AUTHENTICATION, 'app-tokenify-jwt');
  }
  ...
  if (lodash.isArray(protectedPaths_kst) && !lodash.isEmpty(protectedPaths_kst)) {
    webserverTrigger.inject(params.tokenifyService.verifyKST,
        protectedPaths_kst, position.POSITION_AUTHENTICATION, 'app-tokenify-kst');
  }
  ...
  if (lodash.isArray(protectedPaths_httpauth) && !lodash.isEmpty(protectedPaths_httpauth)) {
    webserverTrigger.inject(params.tokenifyService.verifyHttpAuth,
        protectedPaths_httpauth, position.POSITION_AUTHENTICATION, 'app-tokenify-httpauth');
  }
  ...
  if (lodash.isArray(protectedPaths) && !lodash.isEmpty(protectedPaths)) {
    webserverTrigger.inject(params.tokenifyService.verifyToken,
        protectedPaths, position.POSITION_AUTHENTICATION, 'app-tokenify-restrict');
  }
