...
var Service = function (params) {
  ...
  var defaultAuthMethods = ['token', 'httpauth', 'jwt', 'kst'];
  ...
  self.verifyToken = function (req, res, next) {
    if (tokenCfg.enabled == false && process.env.NODE_ENV != 'production') {
      debuglog.isEnabled && debuglog(' - The Token verification is bypassed in NODE_ENV[%s]', process.env.NODE_ENV);
      self.logger.debug('The Token verification is bypassed in NODE_ENV[%s] - Request[%s]', process.env.NODE_ENV, req.traceRequestId);
      req[pluginCfg.sessionObjectName] = { enabled: false };
      return next();
    }
    verifyToken(req).then(function () {
      self.logger.debug('Token verification passed - Request[%s]', req.traceRequestId);
      next();
    }).catch(function (error) {
      debuglog.isEnabled && debuglog(' - Token verification failed, return an error code 401');
      self.logger.debug('Token verification failed, return 401 - Request[%s]', req.traceRequestId);
      if (lodash.isObject(error)) {
        res.status(401).json(error);
      } else {
        res.status(401).send({ success: false, message: error });
  ...
  var verifyToken = verifier['token'] = function (req) {
    debuglog.isEnabled && debuglog(' - check credentials token');
