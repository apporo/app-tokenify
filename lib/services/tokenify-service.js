...
var Service = function(params) {
  ...
  self.authenticate = function(req, res, next) {
    ...
    params.tokenifyStorage.authenticate({
      ...
      if (result.status == 0) {
        ...
        var token = jwt.sign(tokenObject, jwtCfg.password || 't0ps3cr3t', {
  ...
  self.verifyToken = function(req, res, next) {
    ...
    if (token) {
      ...
      jwt.verify(token, jwtCfg.password || 't0ps3cr3t', tokenOpts, function(err, decoded) {
        if (err) {
      ...
      if (pluginCfg.enabled == false) {
        debuglog.isEnabled && debuglog(' - Token not found, but the verification is bypassed');
        self.logger.debug('Token not found, but the verification is bypassed - Request[%s]', req.traceRequestId);
        return next();
      }
