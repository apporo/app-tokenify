...
var Service = function(params) {
  ...
  var mixCfg = methodCfg['mix'] = lodash.get(pluginCfg, ['mix'], {});
  ...
  debuglog.isEnabled && debuglog(' - appTokenify.mixCfg: %s', JSON.stringify(mixCfg));
  ...
  if (lodash.isArray(mixCfg.authMethods) && !lodash.isEmpty(mixCfg.authMethods)) {
    mixtureAuthMethods = lodash.intersection(mixtureAuthMethods, mixCfg.authMethods);
  }
  ...
  self.verifyMIX = function(req, res, next) {
    if ((pluginCfg.enabled == false || mixCfg.enabled == false) && process.env.NODE_ENV != 'production') {
      debuglog.isEnabled && debuglog(' - The MIX verification is bypassed in NODE_ENV[%s]', process.env.NODE_ENV);
      self.logger.debug('The MIX verification is bypassed in NODE_ENV[%s] - Request[%s]', process.env.NODE_ENV, req.traceRequestId);
      req[pluginCfg.sessionObjectName] = { enabled: false };
      return next();
    }
    self.logger.debug('Start MIX verification with authMethods: %s - Request[%s]', JSON.stringify(mixtureAuthMethods), req.traceRequestId);
