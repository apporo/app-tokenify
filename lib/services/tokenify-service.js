...
var Service = function(params) {
  ...
  self.verifyToken = function(req, res, next) {
    ...
    Promise.any([ verifyJWT(req), verifyKST(req) ]).then(function() {
      ...
      debuglog.isEnabled && debuglog(' - The MIX verification failed, return an error code 403');
      self.logger.debug('The MIX verification failed, return 403 - Request[%s]', req.traceRequestId);
      res.status(403).send({
        success: false,
