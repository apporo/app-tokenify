Feature: Mix Authentication
  ...
  Scenario: Request with basic authentication using static username/password
    When I send a request 'GET' to '/tokenify/mix/authorized' with username 'invaliduser' and password 'secretpassword' in 'basic' mode
    Then the response has statusCode '401' and contains the object '{ "success": false }'
    When I send a request 'GET' to '/tokenify/mix/authorized' with username 'static1' and password 'dobietday' in 'basic' mode
  ...
  Scenario: Request with basic authentication using valid username/password from REST API
    ...
    When I send a request 'GET' to '/tokenify/mix/authorized' with username 'mycompany/apiuser' and password 'dobietday' in 'basic' mode
    Then the response has statusCode '200' and contains the object '{ "status": 200, "message": "authorized" }'
    When I send a request 'GET' to '/tokenify/mix/session-info' with username 'mycompany/apiuser' and password 'dobietday' in 'basic' mode
  ...
  Scenario: Request with MIX authentication using static username/password
    ...
    When I send a request 'GET' to '/tokenify/mix/session-info' with received token
    Then the response has statusCode '200' and contains the object '{"user":{"username":"operator","store":"fileEntrypointStore"}}'
    When I send a request 'GET' to '/tokenify/mix/authorized' with received token
    Then the response has statusCode '200' and contains the object '{ "status": 200, "message": "authorized" }'
    When I send a request 'GET' to '/tokenify/mix/authorized'
    Then the response has statusCode '401' and contains the object '{ "success": false }'
